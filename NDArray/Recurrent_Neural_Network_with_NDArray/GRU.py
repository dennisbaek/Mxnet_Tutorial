...
def GRU(epoch = 100 , batch_size=100, save_period=100 , load_period=100 ,learning_rate= 0.1, ctx=mx.gpu(0)):
    ...
    if os.path.exists(path):
        ...
        wxz = nd.random_normal(loc=0, scale=0.01, shape=(num_hidden, num_inputs) , ctx=ctx)
        wxr = nd.random_normal(loc=0, scale=0.01, shape=(num_hidden, num_inputs), ctx=ctx)
        wxh = nd.random_normal(loc=0, scale=0.01, shape=(num_hidden, num_inputs), ctx=ctx)

        whz = nd.random_normal(loc=0, scale=0.01, shape=(num_hidden, num_hidden), ctx=ctx)
        whr = nd.random_normal(loc=0, scale=0.01, shape=(num_hidden, num_hidden) ,ctx=ctx)
        whh = nd.random_normal(loc=0, scale=0.01, shape=(num_hidden, num_hidden), ctx=ctx)

        bz = nd.random_normal(loc=0,scale=0.01,shape=(num_hidden,) , ctx=ctx)
        br = nd.random_normal(loc=0,scale=0.01,shape=(num_hidden,) , ctx=ctx)
        bh = nd.random_normal(loc=0,scale=0.01,shape=(num_hidden,) , ctx=ctx)
